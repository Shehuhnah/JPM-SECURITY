...
function GuardAttendanceRecords() {
  ...
  const kpis = useMemo(() => {
    ...
    // 2. Aggregate calculations over filtered list
    let totalMinutes = 0;
    let completedCount = 0;
    const clientCounts = {};

    filteredRecords.forEach((r) => {
      if (r.workSummary?.totalHours) {
        const hrs = parseFloat(r.workSummary.totalHours);
        if (!Number.isNaN(hrs)) totalMinutes += hrs * 60;
      } else if (r.timeIn && r.timeOut) {
        const start = new Date(r.timeIn);
        const end = new Date(r.timeOut);
      ...
      const clientName = r.scheduleId?.client || "Unassigned Site";
